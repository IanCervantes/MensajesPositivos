package com.example.iance.mensajespositivos;
...
public class MainActivity extends AppCompatActivity {
    ...
    public void atras(View view){
        if (posicion==0){
            posicion =mensajes.length-1;
            textView.setText(mensajes[posicion]);
        }else{
            posicion=posicion-1;
            textView.setText(mensajes[posicion]);
        }
    ...
    public void buscar(View view){
        posicion=Integer.parseInt(editText.getText().toString());
        if (posicion>=0 && posicion<=(mensajes.length-1)){
            textView.setText(mensajes[posicion]);
        }else{
            Toast.makeText(  this, "Debe ingresar un numero que coincida con el arreglo", Toast.LENGTH_LONG).show();
        }
    }
